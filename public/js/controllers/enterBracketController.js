angular.module('enterBracketCtrl', []).controller('enterBracketController', function($scope, dashboard, $http) {
...
var groupA_bracket = new Bracket("A1","A2",false);
$scope.groupA_bracket = groupA_bracket;
var groupB_bracket = new Bracket("B1","B2",false);
$scope.groupB_bracket = groupB_bracket;
var groupC_bracket = new Bracket("C1","C2",false);
$scope.groupC_bracket = groupC_bracket;
var groupD_bracket = new Bracket("D1","D2",false);
$scope.groupD_bracket = groupD_bracket;
var groupE_bracket = new Bracket("E1","E2",false);
$scope.groupE_bracket = groupE_bracket;
var groupF_bracket = new Bracket("F1","F2",false);
$scope.groupF_bracket = groupF_bracket;
var groupG_bracket = new Bracket("G1","G2",false);
$scope.groupG_bracket = groupG_bracket;
var groupH_bracket = new Bracket("H1","H2",false);
...
calculateGroupWinners = function(id1,id2,id3,id4,id5,id6,team1_pts,team2_pts,team3_pts,team4_pts,groupBracket){
	...
	$scope[groupBracket].exception = false;
	...
	var possibleWinners = [];
	var t1 = $scope.groupOutcome[0][1];
	possibleWinners.push($scope.groupOutcome[0][0]);
	for (var i=1; i<4; i++){
		if($scope.groupOutcome[i][1] == t1){
			possibleWinners.push($scope.groupOutcome[i][0])
		}
	}
	console.log(possibleWinners);
	//DETERMINE GROUP WINNERS -------------------------

	//If a team has the most points, group1st
	if ($scope.groupOutcome[0][1] > $scope.groupOutcome[1][1]){
		...
		$scope[groupBracket].exception = true;
		$("." + groupBracket.exception).html(possibleWinners);
	...
	//Top 2 teams (at least) are tied, raise exception
		...
		if($scope.groupOutcome[0][1] == $scope.groupOutcome[1][1]){
			...
			$scope[groupBracket].exception = true;
			$("." + groupBracket.exception).closest(".exception").html(possibleWinners);
...
isAllGroupPicked();
// console.log($scope.allGames[id].winnerPick);
